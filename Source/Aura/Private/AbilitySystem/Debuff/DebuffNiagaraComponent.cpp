...
{
	const bool IsOwnerValid = IsValid(GetOwner());
	const bool IsOwnerAlive = GetOwner()->Implements<UCombatInterface>() && !ICombatInterface::Execute_IsDead(GetOwner());

	if (NewCount > 0 && IsOwnerValid && IsOwnerAlive)
	{
		Activate();
	}
	else
